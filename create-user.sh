...
while IFS="$seperator" read -r username name surname password groups; do
  ...
  #create user
  sudo samba-tool user create "$username" "$password" --given-name="$name" --surname="$surname" --userou="$ouPath" && echo "created user $username" || echo "failed to create user $username"
